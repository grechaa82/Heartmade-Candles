...
public class GetPaidOrderHandler : CallBackQueryHandlerBase
{
    ...
    public override bool ShouldHandleUpdate(CallbackQuery callbackQuery, TelegramUser user)
    {
        ...
        return text.Contains(CallBackQueryCommands.PaidOrderNextCommand) 
            || text.Contains(CallBackQueryCommands.PaidOrderPreviousCommand) 
            || text.Contains(CallBackQueryCommands.PaidOrderSelectCommand);
    }
    ...
    public async override Task Process(CallbackQuery callbackQuery, TelegramUser user)
    {
        ...
        if(callbackQuery.Data.ToLower().Contains(CallBackQueryCommands.PaidOrderSelectCommand))
        {
        ...
        var (orderMaybe, totalCount) = await GetOrderByStatusAndTotalCount(OrderStatus.Paid, 1, pageIndex - 1);
        ...
        var inlineKeyboard = OrderInlineKeyboardMarkup.GetOrderSelectionMarkup(
            previousCommands: CallBackQueryCommands.PaidOrderPreviousCommand,
            nextCommands: CallBackQueryCommands.PaidOrderNextCommand,
            selectCommands: CallBackQueryCommands.PaidOrderSelectCommand,
